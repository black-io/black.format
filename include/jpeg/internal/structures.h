...
namespace Internal
{
	...
	*/
	...
	struct ScanFooter final
	{
		...
		uint8_t		section_end;		// (Se) End of spectral selection.
	...
	/**
		@breif	Component of SOS segment in regular JIF file.
	...
	*/
	struct ImageBlockEntry final
	{
		...
		Black::PlainView<const std::byte>		image;				// Image block right after the SOS segment.
	};
}
}
}
